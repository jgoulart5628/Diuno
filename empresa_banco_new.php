 ...
 <?php
  ...
  function INC_ALT_EXC($id,$dbms,$oper) {
     ...
     $tela = '<div class="form-group">
                ID : <input type="text" class="entra" id="lid" name="id"  value="'.$id.'" placeholder="Nome reduzido para acesso e chave">
  ...
  function TESTAR($dados) {
     ...
     $driver        = $dados['driver'];
     $banco         = $dbms.'_'.$id;
     $con = new banco_dados($banco, $resp);
     $xx = get_object_vars($con); 
     if ($xx['erro'])  {
         $resp->alert("Não Conectou!".$xx['erro'].'-'.$banco); } else { 
 ...
 function consulta_clientes() {
    ...
    if (count($res) > 0) {
       foreach ($res as $cli) {
	     ...
	     $driver        = $cli['driver'];
	     $tela .= '<tr>
      	<td data-field="id" align="left" data-sortable="true"><input type="button" style="width: 120px; text-align: left;" onclick="xajax_INC_ALT_EXC(\''.$id.'\',\''.$dbms.'\',\'A\'); return false;" value="'.$id.'"></td>
