...
//    $resp->alert($dbms); 
 ...
 function Carga($dados) { 
   ...
   if (substr($pastax,0,3) === 'SQL') {  $pasta = $pastax.$cliente.'/'; $opc = 'sql'; } else { $pasta = $pastax; }
   ...
   foreach (new DirectoryIterator($pasta) as $fileInfo) {
     ...
     $arqs  = explode('.',$fileInfo->getFilename());
     $ext = $arqs[1];
     if ($ext !== 'bat')  {
   ...
   if(count($file) > 0)  {
     if ($file === 'sql')  {
        foreach($file as $arq)  {
          $tela .= '<li class="active"><a href="#" style="text-align:left;">'.$arq.'</a>
                 </li>';
        }         
     } else {
          foreach($file as $arq)  {
             $tela .= '<li class="active"><a href="#" style="text-align:left;" onclick="xajax_Mostra_SQL(\''.$pasta.'/'.$arq.'\'); return true;">'.$arq.'</a>
                 </li>';
          }       
     }         
  }
...
//  $sql = 'Teste de SQL';
  $tela .= '<label for="textarea">'.$arq.'</label>
              ...
              </br><button class="btn btn-sm btn-success padrao" onclick="xajax_Executar(xajax.getFormValues(\'tela\')); return false;");"> Executar&nbsp;<span class="glyphicon glyphicon-play"></span></button>
