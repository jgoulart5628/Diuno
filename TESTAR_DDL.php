...
//    $telax = busca_dados(PASTA);     
 ...
 function combo_dbms($dbms='')  {
     $dbs = array('ORACLE','SQLSERVER','FIREBIRD','MYSQL','ODBC','SQLITE','POSTGRESQL');
...
//    $resp->alert($dbms); 
 ...
 function Carga($dados) { 
   ...
   $banco         = $dbms.'_'.$cliente;
   if ($dbms === 'ODBC')  { $conv  = '1'; }
   if ($dbms === 'FIREBIRD')  { $conv  = '3'; }
   if (!$cliente || !$dbms)  { $resp->alert("Para executar escolha um DBMS!"); return $resp; }
   $en  = new InOut_model($banco);
   $tabs =  $en->Lista_Tabelas_Orig($conv);
   $tela = '<div id="arq_sql" class="row">';
   $tela .= '<div id="pendente"><h4>Tabelas do Cliente'.$pasta.'</h4>
               <ul id="sidebar" class="nav nav-pills nav-stacked" style="max-width: 320px; margin-left: 5px;">';
   if(count($tabs) > 0)  {
     foreach($tabs as $tabx)  {
       $tabela = $tabx['name'];
       if (!$tabela) { $tabela  = $tabx['NAME']; }
       $tela .= '<li class="active"><a href="#" style="text-align:left;" onclick="xajax_Mostra_SQL(\''.$banco.'\',\''.$tabela.'\', \''.$conv.'\');">'.$tabela.'</a>
                 </li>';
     }
   } 
...
//   $tela .= '</div>';
 ...
 function Mostra_SQL($banco, $tabela, $conv) {
  $resp     = new xajaxResponse();
  $en  = new InOut_model($banco);
  $tbs = $en->DDL_Origem(strtoupper($tabela), $conv);
//  $resp->alert(print_r($tbs,true).' - '.$banco.' - '.$tabela); return $resp;
  $sql = '';
  if (count($tbs) > 0)   {
    foreach ($tbs as $tab)  {
       if ($conv === '1' || $conv === '2') {
       ...
       $lin   = $campo.' '.$tipo.' '.$tam.' '.$decx.' '.$nulo.' '.$pk;
       $sql  .= $lin."\r\n";
    }
  } 
  $tela = '<div class="form-group"> 
            <h3>SQL</h3></br>';
//  $sql = 'Teste de SQL';
  $tela .= '<label for="textarea">'.$tabela.'</label>
